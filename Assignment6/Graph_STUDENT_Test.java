...
public class Graph_STUDENT_Test{
	...
	public void setUp() throws Exception {
		 graph = new Graph();
		  ...
		  graph.addEdge(town[1], town[2], 4, "Maine");
		  graph.addEdge(town[1], town[3], 5, "Atlanta");
		  graph.addEdge(town[1], town[5], 7, "Rockville");
		  graph.addEdge(town[3], town[4], 4, "Riverdale");
		  graph.addEdge(town[3], town[2], 4, "Carolton");
		  graph.addEdge(town[4], town[5], 8, "Bladensburg");
...
//		  graph.addEdge(town[2], town[11], 6, "Road_12");
	...
	public void testGetEdge() {
		assertEquals(new Road(town[1], town[3],5, "Atlanta"), graph.getEdge(town[1], town[3]));
		assertEquals(new Road(town[3], town[2],4, "Carolton"), graph.getEdge(town[3], town[2]));
	}

	@Test
	public void testAddEdge() {
		assertEquals(false, graph.containsEdge(town[3], town[5]));
		graph.addEdge(town[3], town[5], 1, "Georgia");
	...
	public void testEdgeSet() {
		...
		assertEquals("Atlanta", roadArrayList.get(0));
		assertEquals("Bladensburg", roadArrayList.get(1));
		assertEquals("Carolton", roadArrayList.get(2));
		assertEquals("Maine", roadArrayList.get(3));
	...
	public void testEdgesOf() {
		...
		Collections.sort(roadArrayList);
		assertEquals("Atlanta", roadArrayList.get(0));
		assertEquals("Maine", roadArrayList.get(1));
		assertEquals("Rockville", roadArrayList.get(2));
	}
	
	@Test
	public void testRemoveEdge() {
		assertEquals(true, graph.containsEdge(town[4], town[5]));
		graph.removeEdge(town[4], town[5], 8, "Bladensburg");
	 ...
	 @Test
	  public void testTown_1ToTown_11() {
		  ...
		  {    	
			  ...
			  if(town.getTownName().equals(beginTown))
				  beginIndex = town;
			  if(town.getTownName().equals(endTown))
				  endIndex = town;		
		  }
